...
// DB의 region_id는 AUTO_INCREMENT로 생성됨
const regionStartPoints = {
  1: { lat: 37.5665, lng: 126.9780, name: "명동", type: "관광중심지" },           // 서울
  2: { lat: 35.1584, lng: 129.0598, name: "해운대", type: "관광중심지" },         // 부산
  3: { lat: 33.4996, lng: 126.5312, name: "제주시 중심부", type: "관광중심지" },  // 제주도
  4: { lat: 37.7556, lng: 128.8961, name: "강릉 중앙시장", type: "관광중심지" },  // 강릉
  5: { lat: 35.8562, lng: 129.2247, name: "경주 시내", type: "관광중심지" },      // 경주
  6: { lat: 34.7465, lng: 127.7420, name: "여수 엑스포", type: "관광중심지" },    // 여수
  7: { lat: 35.8154, lng: 127.1530, name: "전주 한옥마을", type: "관광중심지" },  // 전주
  8: { lat: 38.2070, lng: 128.5918, name: "속초 해변", type: "관광중심지" },      // 속초
  9: { lat: 37.4750, lng: 126.6213, name: "인천 차이나타운", type: "관광중심지" }, // 인천
  10: { lat: 36.3504, lng: 127.3845, name: "대전 엑스포", type: "관광중심지" },   // 대전
  11: { lat: 35.8714, lng: 128.6014, name: "대구 동성로", type: "관광중심지" },   // 대구
  12: { lat: 35.1596, lng: 126.8526, name: "광주 충장로", type: "관광중심지" },   // 광주
};
...
const onDragMove = (evt) => {
  ...
  // 2. 이동 대상이 고정된 아이템이면 불가
  if (relatedItem?.fixed) return false;
  
  // 3. 고정된 항목을 경계로 한쪽 블록만 이동 허용
  ...
  // dragged와 target이 같은 블록 안에 있는지 판단
  const inSameBlock = fixedIndexes.every((fi) => {
    // 둘 다 fi보다 작거나, 둘 다 fi보다 크면 같은 블록
    return (
...
onMounted(async () => {
  await loadPlanInfo(); // 먼저 Plan 정보를 가져와서 시작 지점 설정
  initMap(); // 그 다음 지도 초기화
  await loadDurations();
